...
void setup() {
  ...
  // --- [ แก้ไขส่วน WDT สำหรับ ESP32 Core v3.0+ ] ---
  esp_task_wdt_config_t twdt_config = {
      .timeout_ms = 8000,           // ตั้งเวลา 8 วินาที (หน่วยเป็น ms)
      .idle_core_mask = (1 << 0),   // ตรวจสอบที่ Core 0
      .trigger_panic = true         // ถ้าค้างให้ Restart (Panic)
  };
  esp_task_wdt_init(&twdt_config);  // เริ่มต้นด้วยค่า config ใหม่
  esp_task_wdt_add(NULL);           // เพิ่ม Task ปัจจุบัน (loop) เข้าไปตรวจจับ
